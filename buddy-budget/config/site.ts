export type SiteConfig = typeof siteConfig;

export const siteConfig = {
  name: "Buddy Budget",
  description:
    "Your AI-Powered Personal Finance Companion. Take control of your finances with powerful tools for expense tracking, budgeting, investment monitoring, and AI-powered predictions.",
  navItems: [
    {
      label: "Home",
      href: "/",
    },
    {
      label: "Features",
      href: "/#features",
    },
    {
      label: "Net Worth Predictor",
      href: "/net-worth",
    },
    {
      label: "Finance Tracker",
      href: "/finance",
    },
    {
      label: "About",
      href: "/about",
    },
  ],
  navMenuItems: [
    {
      label: "Home",
      href: "/",
    },
    {
      label: "Features",
      href: "/#features",
    },
    {
      label: "Net Worth Predictor",
      href: "/net-worth",
    },
    {
      label: "Finance Tracker",
      href: "/finance",
    },
    {
      label: "Dashboard",
      href: "/dashboard",
    },
    {
      label: "Settings",
      href: "/settings",
    },
    {
      label: "About",
      href: "/about",
    },
  ],
  links: {
    github: "https://github.com/YonderUrik/buddy-budget",
    contributing:
      "https://github.com/YonderUrik/buddy-budget/blob/main/CONTRIBUTING.md",
    sponsor: "https://github.com/sponsors/YonderUrik",
    license: "https://github.com/YonderUrik/buddy-budget/blob/main/LICENSE",
    privacy: "/privacy",
    terms: "/terms",
  },
};
